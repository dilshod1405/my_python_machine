...
with connect("my_table.db") as db:
    cursor = db.cursor()
    cursor.execute(
        """
...
class ContactsRepo:
    ...
    def earn(self, earn, spend):
        cursor = db.cursor()
        cursor.execute(
            '''
            INSERT INTO finance(earn, spend)
            VALUES (?, ?)
            ''',
            (earn, spend,)
        )
...
if __name__ == '__main__':
    ...
    with sqlite3.connect('my_table.db') as db:
        repo = ContactsRepo(db)
        my_table = PrettyTable(['Balance'])
        if command == 'earn':
            earn = int(input('Balansga pul o`tkazish miqdori: '))
            spend = 0
            repo.earn(earn, spend)
            print('Amaliyot muvaffaqiyatli bajarildi. Balansni ko`rish uchun balance buyrug`idan foydalaning')
        elif command == 'spend':
            spend = int(input('Balansdan pul yechish miqdorini kiriting: '))
            earn = 0
            repo.spend(earn, spend)
            print('Amaliyot muvaffaqiyatli bajarildi. Balansni ko`rish uchun balance buyrug`idan foydalaning')
        elif command == 'balance':
            print(max(repo.balance_1()[0]) - max(repo.balance_2()[0]))
